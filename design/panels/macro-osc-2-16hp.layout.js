...
const MID = 40.64;   // the panel's centre line, at 16 HP

export default panel({ hp: 16, pad: 1.2, rules: false }, [
  band(null, [
    placed([
      // ---- the pair across the top, equidistant from the centre line
      { c: knack('freq', 'FREQ', 'fmCv', { depth: 'fmDepth', size: 9.0, scale: HZ }), x: MID - 19.5, y: 26 },
      { c: readout('model', 'MODEL', { widest: 'particle', value: 'string', menu: true, pad: 1.0, size: 2.0, side: 'above' }), x: MID + 19.5, y: 22 },
      { c: jack('modelCv', 'cv'), x: MID + 19.5, y: 35 },
      // Pitch under the frequency knob, clear of its printed scale.
      { c: jack('pitchIn', 'v/oct'), x: 8, y: 52 },
      ...
      { c: knack('harmonics', 'HARM', 'harmonicsCv', { depth: 'harmonicsDepth', size: 8.0 }), x: MID - 21, y: 76 },
      { c: knack('timbre', 'TIMBRE', 'timbreCv', { depth: 'timbreDepth', size: 8.0 }), x: MID + 21, y: 76 },

      // ---- the strike along the foot, in the order it happens
      { c: knob('decay', 'DECAY', { size: 5.0 }), x: 12, y: 98 },
      { c: knob('colour', 'COLOUR', { size: 5.0 }), x: 29, y: 98 },
      { c: jack('trigIn', 'trig'), x: 44, y: 98 },
      { c: jack('levelIn', 'level'), x: 55, y: 98 },
      { c: jack('out', 'out', { labelSize: 2.1 }), x: 66, y: 98 },
      { c: jack('auxOut', 'aux', { labelSize: 2.1 }), x: 74.5, y: 98 },
    ], 112),
  ]),
]);
