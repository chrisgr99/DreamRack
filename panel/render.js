...
import { defs, jack, vjack, knob, knack, trim, slider, button, vuMeter, label, radioGroup, bipolarMark, attachedLabel, ledLamp, stepButton, readout } from './primitives.js';
...
function renderItem(it, th, dark) {
  switch (it.t) {
    ...
    case 'trim':         return trim(it.id, it.x, it.y, themed(it.opts, th, true));
    case 'slider':       return slider(it.id, it.x, themed(it.opts, th, true));
    case 'readout':      return readout(it.id, it.x, it.y, themed(it.opts, th, true));
    case 'button':       return button(it.id, it.x, it.y, themed(it.opts, th, true));
