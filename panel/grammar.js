...
export const SIZE = { large: 9.5, medium: 7.2, small: 5.6, tiny: 4.2 };
const JACK_R = 3.0;
const LAMP_R = 1.5;

const MARGIN = 4.5;          // panel edge to the outermost thing in a row
...
const LABEL_H = 3.2;         // a label below a control
const SUB_H = 2.9;
...
export const knack = (id, label, port, opts = {}) =>
  ({ kind: 'knack', id, label, port, depth: opts.depth || null, size: opts.size || 'medium',
     scale: opts.scale || null, sub: opts.sub || null });

export const jack = (id, label, opts = {}) => ({ kind: 'jack', id, label, r: opts.r || JACK_R });
...
export const band = (header, rows) => ({ header, rows: rows.map((r) => (r.controls ? r : row(r))) });
...
function halfWidth(c) {
  ...
  let art = 0;
  if (c.kind === 'knob' || c.kind === 'knack') art = SIZE[c.size] + (c.scale ? 3.2 : 0);
  ...
  if (c.kind === 'lamps') {
    ...
    const size = c.kind === 'jack' ? JACK_LABEL : LABEL_SIZE;
...
function rowHeight(r) {
  ...
  const hasLabel = r.controls.some((c) => c.label && c.kind !== 'lamps');
...
function emit(items, c, x, y) {
  const lab = c.label ? { text: c.label, placement: 'below', size: c.kind === 'jack' ? JACK_LABEL : LABEL_SIZE, gap: 1.6 } : null;
  switch (c.kind) {
    case 'knob':
...
export function panel(opts, bands) {
  ...
  const need = stacked.map((b) => (b.header ? HEADER_H : 0)
    + b.rows.reduce((s, r) => s + rowHeight(r), 0) + (b.rows.length - 1) * ROW_PAD + BAND_PAD);
  const outH = outBand ? rowHeight(outBand.rows[0]) + 4 : 0;
  const total = need.reduce((s, h) => s + h, 0) + outH;
  const spare = Math.max(0, FACE_H - 4 - total);
  const gaps = stacked.reduce((n, b) => n + (b.rows.length - 1), 0) + stacked.length;
  const extra = gaps ? spare / gaps : 0;
  ...
  stacked.forEach((b, bi) => {
    ...
    y = ry + BAND_PAD + extra;
    if (bi < stacked.length - 1 || outBand) {
